### --- loading data, saving pipeline, loading saved pipeline, remving old pipelines ---
...
def save_pipeline(*, pipeline_to_persist: Pipeline) -> None:
    """
    For Saving pipeline:
     Saves versioned model, and overwrites any previous saved models. 
     This ensures that when package is published, there is only one trained model 
     that can be called, and we know exactly how it was built.
    """
...
def remove_old_pipelines(*, files_to_keep: t.List[str]) -> None:
    ...
    Remove old model pipelines.
     This is to ensure there is a simple one-to-one mapping between package version and
